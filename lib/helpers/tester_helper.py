...
class Tester(object):
    def __init__(self, cfg, model, ema_model, dataloader, logger, train_cfg=None, model_name='monodgp'):
        self.cfg = cfg
        self.model = model
    ...
    def test(self):
        ...
        if self.cfg['mode'] == 'single' or not self.train_cfg["save_all"]:
            ...
            if self.ema_model is not None:
                load_checkpoint(model=self.ema_model,
                                optimizer=None,
    ...
    def inference(self):
        ...
        module = self.ema_model.module if self.ema_model is not None else self.model
        module.eval()
        ...
        for batch_idx, (inputs, calibs, targets, info) in enumerate(self.dataloader):
            ...
            ###dn
            outputs = module(inputs, calibs, targets, img_sizes, dn_args = 0)
            ###
            end_time = time.time()
