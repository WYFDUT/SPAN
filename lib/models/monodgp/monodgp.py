...
from lib.losses.iou3d_loss import boxes3d_corner
from lib.losses.proj_loss import boxes3d_proj2d_loss
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, focal_alpha, losses, inter_losses, group_num=11):
        ...
        self.loss_mgiou = MGIoU3D(reduction="none")
    ...
    def loss_3dmgiou(self, outputs, targets, indices, num_boxes):
        idx = self._get_src_permutation_idx(indices)
        out_corners, tgt_corners = boxes3d_corner(outputs, targets, idx, indices)
        losses = {}
        mgiou_loss = 0
        if tgt_corners is None:
            mgiou_loss += torch.tensor(0.0).to(outputs["pred_boxes"].device)
            losses['loss_threediou'] = mgiou_loss.sum()
            return losses
        mgiou_loss += self.loss_mgiou(out_corners, tgt_corners)
        losses['loss_threediou'] =  mgiou_loss.sum() / num_boxes
        return losses
    
    def loss_proj(self, outputs, targets, indices, num_boxes):
        idx = self._get_src_permutation_idx(indices)
        proj_loss = 0
        proj_loss += boxes3d_proj2d_loss(outputs, targets, idx, indices)
        losses = {}
        if num_boxes == 0: losses['loss_proj'] = proj_loss
        else: losses['loss_proj'] = proj_loss / num_boxes
        return losses
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        for i, layer in enumerate(self.layers):
            x = F.mish(layer(x)) if i < self.num_layers - 1 else layer(x)
        return x
...
def build(cfg):
    ...
    weight_dict['loss_threediou'] = cfg['mgiou_loss_coef']
    weight_dict['loss_proj'] = cfg['proj_loss_coef']
    ...
    losses = ['labels', 'boxes', 'cardinality', 'depths', 'dims', 'angles', 'center', 'depth_map', 'region', 'threediou', 'proj']
