...
class KITTI_Dataset(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        calibs_perimg = np.zeros((1, 3, 4), dtype=np.float32)
        rotation_y = np.zeros((self.max_objs, 1), dtype=np.float32)
        calibs_perimg[0] = calib.P2
        ...
        if random_mix_flag == True:
            # if False:
                ...
                for i in range(object_num_temp):
                    ...
                    rotation_y[i + object_num] = objects[i].ry
